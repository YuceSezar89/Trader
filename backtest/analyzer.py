...
class PerformanceAnalyzer:
    ...
    def analyze_trade_distribution(self, trades: List[Dict]) -> Dict:
        ...
        try:
            ...
            analysis = {
                ...
                # Oranlar
                "avg_win_loss_ratio": (
                    abs(np.mean(winning_trades) / np.mean(losing_trades))
                    if winning_trades and losing_trades
                    else 0
                ),
                "largest_win_loss_ratio": (
                    abs(max(winning_trades) / min(losing_trades))
                    if winning_trades and losing_trades
                    else 0
                ),
            }
    ...
    def generate_comprehensive_report(
        self, trades: List[Dict], initial_balance: float = 10000
    ) -> Dict:
        """
    ...
    def _calculate_performance_grade(
        self, win_rate: float, profit_factor: float, sharpe_ratio: float, max_drawdown: float
    ) -> Dict:
        """
