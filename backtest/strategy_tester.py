...
class StrategyTester:
    ...
    async def run_momentum_period_analysis(
        ...
        # En iyi periyodu bul
        best_period = (
            max(period_results, key=lambda x: x["sharpe_ratio"]) if period_results else None
        )
    ...
    async def run_vmp_threshold_analysis(
        ...
        # En iyi eşiği bul
        best_threshold = (
            max(threshold_results, key=lambda x: x["sharpe_ratio"]) if threshold_results else None
        )
    ...
    def _generate_parameter_combinations(
        self, test_params: Dict, max_combinations: int
    ) -> List[Dict]:
        """Parametre kombinasyonlarını oluştur"""
        try:
    ...
    async def _test_parameter_combination(
        self, params: Dict, symbols: List[str], start_date: datetime, end_date: datetime
    ) -> Optional[Dict]:
    ...
    def _analyze_strategy_comparison(self, strategy_results: List[Dict]) -> Dict:
        ...
        try:
            ...
            best_win_rate = max(
                strategy_results, key=lambda x: x["performance_metrics"]["win_rate"]
            )
            best_sharpe = max(
                strategy_results, key=lambda x: x["performance_metrics"]["sharpe_ratio"]
            )
            best_profit = max(strategy_results, key=lambda x: x["performance_metrics"]["total_pnl"])
            lowest_drawdown = min(
                strategy_results, key=lambda x: x["performance_metrics"]["max_drawdown"]
            )
            ...
            return {
                ...
                "overall_winner": (
                    strategy_results[0]["strategy_name"] if strategy_results else None
                ),
                "summary_stats": {
                    "avg_win_rate": sum(
                        r["performance_metrics"]["win_rate"] for r in strategy_results
                    )
                    / len(strategy_results),
                    "avg_sharpe_ratio": sum(
                        r["performance_metrics"]["sharpe_ratio"] for r in strategy_results
                    )
                    / len(strategy_results),
                    "avg_max_drawdown": sum(
                        r["performance_metrics"]["max_drawdown"] for r in strategy_results
                    )
                    / len(strategy_results),
                },
            }
    ...
    def _analyze_momentum_periods(self, period_results: List[Dict]) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "trends": {
                    "win_rate_trend": (
                        "increasing" if win_rates[-1] > win_rates[0] else "decreasing"
                    ),
                    "sharpe_trend": (
                        "increasing" if sharpe_ratios[-1] > sharpe_ratios[0] else "decreasing"
                    ),
                },
                "optimal_range": {
    ...
    def _generate_momentum_recommendations(self, period_results: List[Dict]) -> List[str]:
        ...
        try:
            ...
            recommendations.append(
                f"En iyi performans: {best_period['momentum_period']} bar (Sharpe: {best_period['sharpe_ratio']:.2f})"
            )
            ...
            if short_periods and long_periods:
                ...
                if avg_short_win > avg_long_win:
                    recommendations.append(
                        "Kısa periyotlar (1-3 bar) daha yüksek win rate sağlıyor"
                    )
                else:
            ...
            recommendations.append(
                f"En düşük risk: {low_drawdown['momentum_period']} bar (Drawdown: {low_drawdown['max_drawdown']:.2f}%)"
            )
    ...
    def _analyze_vmp_thresholds(self, threshold_results: List[Dict]) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "threshold_effects": {
                    "low_threshold_effect": (
                        "Daha fazla sinyal, düşük kalite"
                        if trade_counts[0] > trade_counts[-1]
                        else "Beklenmedik sonuç"
                    ),
                    "high_threshold_effect": (
                        "Daha az sinyal, yüksek kalite"
    ...
    def _generate_threshold_recommendations(self, threshold_results: List[Dict]) -> List[str]:
        ...
        try:
            ...
            recommendations.append(
                f"Optimal eşik: {best_threshold['vmp_threshold']} (Sharpe: {best_threshold['sharpe_ratio']:.2f})"
            )

            # Trade frekansı analizi
            high_freq = max(threshold_results, key=lambda x: x["total_trades"])
            recommendations.append(
                f"En yüksek sinyal frekansı: {high_freq['vmp_threshold']} ({high_freq['total_trades']} sinyal)"
            )

            # Kalite analizi
            high_quality = max(threshold_results, key=lambda x: x["win_rate"])
            recommendations.append(
                f"En yüksek kalite: {high_quality['vmp_threshold']} (%{high_quality['win_rate']:.1f} win rate)"
            )
