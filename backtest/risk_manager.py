...
class RiskManager:
    ...
    def calculate_position_size(
        self, account_balance: Decimal, entry_price: float, risk_percentage: Optional[float] = None
    ) -> Decimal:
        """
        ...
        try:
            ...
            logger.debug(
                f"Position size: {quantity} (Risk: {risk_pct}%, Value: {max_position_value})"
            )
            return quantity
    ...
    def check_risk_limits(
        self, account_balance: Decimal, proposed_trade_value: Decimal
    ) -> Dict[str, bool]:
        """
        ...
        try:
            ...
            max_position_value = account_balance * Decimal(
                str(self.max_position_size_percentage / 100)
            )
            if proposed_trade_value > max_position_value:
                checks["position_size_ok"] = False
                logger.warning(
                    f"Pozisyon büyüklüğü limiti aşıldı: {proposed_trade_value} > {max_position_value}"
                )
            ...
            if abs(self.daily_pnl) > max_daily_loss:
                ...
                logger.warning(
                    f"Günlük kayıp limiti aşıldı: {abs(self.daily_pnl)} > {max_daily_loss}"
                )
            ...
            if self.open_positions_count >= self.max_open_positions:
                ...
                logger.warning(
                    f"Maksimum pozisyon sayısı aşıldı: {self.open_positions_count} >= {self.max_open_positions}"
                )

            # Genel risk kontrolü
            checks["overall_risk_ok"] = all(
                [checks["position_size_ok"], checks["daily_loss_ok"], checks["max_positions_ok"]]
            )
    ...
    def get_risk_summary(self, account_balance: Decimal) -> Dict:
        ...
        return {
            ...
            "position_slots_used_percentage": (self.open_positions_count / self.max_open_positions)
            * 100,
            "risk_parameters": {
...
class AdvancedRiskManager(RiskManager):
    ...
    def calculate_volatility_adjusted_position_size(
        ...
        try:
            ...
            volatility_multiplier = max(
                0.1, min(2.0, volatility_multiplier)
            )  # 0.1x - 2.0x arası sınırla

            adjusted_size = base_size * Decimal(str(volatility_multiplier))

            logger.debug(
                f"Volatilite ayarlı pozisyon: {adjusted_size} (Çarpan: {volatility_multiplier:.2f})"
            )
    ...
    def calculate_correlation_adjusted_exposure(
        self, current_positions: Dict, new_symbol: str, correlation_matrix: Dict
    ) -> float:
        """
