...
class BacktestEngine:
    ...
    async def _generate_signal(
        self, symbol: str, historical_data: pd.DataFrame, timestamp: datetime, timeframe: str
    ) -> Optional[Dict]:
    ...
    def _calculate_vmp_score(self, data: pd.DataFrame, metrics: Dict) -> float:
        ...
        try:
            ...
            price_change = (data["close"].iloc[-1] - data["close"].iloc[-6]) / data["close"].iloc[
                -6
            ]
    ...
    async def _close_position(
        self, symbol: str, exit_price: float, timestamp: datetime, exit_reason: Optional[str] = None
    ):
        """Pozisyon kapat"""
        if symbol not in self.positions:
            return
