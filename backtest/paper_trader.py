...
class PaperTrader:
    ...
    async def start_session(self) -> int:
        ...
        try:
            async with get_session() as session:
                ...
                logger.info(
                    f"Paper trading oturumu başlatıldı: {self.session_name} (ID: {self.session_id})"
                )
    ...
    async def _process_signal(self, signal):
        ...
        try:
            ...
            risk_checks = self.risk_manager.check_risk_limits(
                self.current_balance, Decimal(str(current_price * 100))  # Örnek trade değeri
            )
    ...
    async def _close_position(
        self, symbol: str, exit_price: float, exit_reason: Optional[str] = None
    ):
        """Pozisyon kapat"""
        try:
            if symbol not in self.positions:
