...
def main():
    ...
    parser.add_argument("--hours", type=int, default=24, help="Kaç saat geriye bak (default: 24)")
    parser.add_argument(
        "--max-signals", type=int, default=None, help="Maksimum sinyal sayısı (default: sınırsız)"
    )
    parser.add_argument("--test", action="store_true", help="Test modu (sadece 10 sinyal)")
    parser.add_argument(
        "--dry-run", action="store_true", help="Hesaplamaları yap ama DB yazma (sadece logla)"
    )
    parser.add_argument("--signal-id", type=int, default=None, help="Tek bir sinyal ID güncelle")
        ...
        with SignalPerformanceAnalyzer() as analyzer:
            ...
            if args.signal_id:
                ...
                stats = analyzer.batch_update_performance(
                    hours_back=hours, max_signals=max_signals, dry_run=args.dry_run
                )
