...
=============================
Sinyal performans KPI'larını görselleştirir.
"""
...
def get_performance_summary(days_back: int = 7):
    """
    Genel performans özetini getirir.
    
    Returns:
        dict: {
            'total_signals': int,
            'calculated_signals': int,
            'hit_rate': float,
            'avg_return_atr': float,
            'avg_return_pct': float,
            'avg_risk_reward': float,
            'total_profit_atr': float
        }
    """
...
@st.cache_data(ttl=60)
def get_performance_by_signal_type(days_back: int = 7):
...
@st.cache_data(ttl=60)
def get_performance_timeseries(days_back: int = 7):
...
@st.cache_data(ttl=60)
def get_top_performers(days_back: int = 7, limit: int = 10):
...
def render_signal_performance_dashboard():
    ...
    # KPI Kartları
    summary = get_performance_summary(days_back)
    ...
    timeseries = get_performance_timeseries(days_back)
    render_performance_chart(timeseries)
    
    st.divider()
    
    # Breakdown Grafikleri
    by_type = get_performance_by_signal_type(days_back)
    by_interval = get_performance_by_interval(days_back)
    ...
    top_performers = get_top_performers(days_back, limit=10)
