...
class TestRedisMTFCache:
    ...
    async def test_set_get_mtf_klines_basic(self, test_symbol, sample_5m_data, clean_redis_state):
        ...
        for col in ["open", "high", "low", "close", "volume"]:
            assert (
                cached_df[col].equals(sample_5m_data[col])
                or (cached_df[col] - sample_5m_data[col]).abs().max() < 0.001
            ), f"Column {col} data should match"

    @pytest.mark.redis
    @pytest.mark.asyncio
    async def test_set_get_mtf_klines_with_limit(
        self, test_symbol, sample_1m_data, clean_redis_state
    ):
        ...
        for col in ["open", "high", "low", "close", "volume"]:
            assert (
                cached_df[col].equals(expected_df[col])
                or (cached_df[col] - expected_df[col]).abs().max() < 0.001
            )
    ...
    @pytest.mark.redis
    @pytest.mark.asyncio
    async def test_flush_mtf_cache(
        self, test_symbol, sample_1m_data, sample_5m_data, clean_redis_state
    ):
    ...
    async def test_warm_mtf_cache(self, test_symbol, clean_redis_state):
        ...
        mtf_data = {"1m": sample_1m, "5m": sample_5m, "15m": sample_15m}
...
class TestRedisMTFCachePerformance:
    ...
    async def test_cache_write_performance(
        self, test_symbol, sample_5m_data, performance_thresholds, clean_redis_state
    ):
        ...
        assert (
            execution_time_ms < performance_thresholds["cache_write_time_ms"]
    ...
    async def test_cache_read_performance(
        ...
        assert (
            execution_time_ms < performance_thresholds["cache_read_time_ms"]
    ...
    async def test_concurrent_cache_operations(self, test_symbol, clean_redis_state):
        ...
        assert (
            execution_time < 1.0
        ), f"Concurrent operations took {execution_time:.2f}s, should be < 1s"
