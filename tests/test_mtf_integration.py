...
class TestMTFIntegration:
    ...
    async def test_mtf_data_consistency(self, test_symbol, clean_redis_state):
        ...
        assert (
            abs(total_volume_1m - total_volume_15m) < 0.01
        ), "1m and 15m volume totals should match"
    ...
    async def test_mtf_multiple_symbols(self, clean_redis_state):
        ...
        for symbol in symbols:
            ...
            for tf in timeframes:
                assert stats[tf]["live_kline_data"][
                    "exists"
                ], f"Cache should exist for {symbol}:{tf}"
    ...
    async def test_mtf_indicators_and_signals_integration(self, test_symbol, clean_redis_state):
        ...
        assert (
            abs(cached_indicators["ema_21"] - df_5m["close"].mean()) < 0.01
        ), "Indicator should match calculated value"
        assert (
            abs(cached_signals[0]["price"] - df_5m["close"].iloc[-1]) < 0.01
        ), "Signal price should match last close"
...
class TestMTFScalability:
    ...
    async def test_high_volume_mtf_operations(self, clean_redis_state):
        ...
        assert (
            successful_retrievals >= total_operations * 0.9
        ), "At least 90% of retrievals should succeed"
        assert (
            retrieval_time < 2.0
        ), f"High volume retrieval took {retrieval_time:.2f}s, should be < 2s"
    ...
    async def test_memory_efficiency_mtf(self, test_symbol, clean_redis_state):
        ...
        assert (
            memory_increase < 200
        ), f"Memory increased by {memory_increase:.2f}MB, should be < 200MB"
