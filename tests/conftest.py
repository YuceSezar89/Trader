...
def minimal_valid_dataframe():
    ...
    return pd.DataFrame(
        {
            "open_time": [
                int((base_time + timedelta(minutes=i)).timestamp() * 1000) for i in range(3)
            ],
...
@pytest.fixture(
    params=[("1m", "5m"), ("1m", "15m"), ("1m", "1h"), ("5m", "15m"), ("5m", "1h"), ("15m", "1h")]
)
...
def assert_aggregation_valid(original_df: pd.DataFrame, aggregated_df: pd.DataFrame, ratio: int):
    ...
    assert (
        len(aggregated_df) == expected_rows
    ), f"Expected {expected_rows} rows, got {len(aggregated_df)}"
