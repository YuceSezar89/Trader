...
from tests.conftest import assert_aggregation_valid, assert_dataframe_valid
...
class TestTimeframeAggregator:
    ...
    def test_can_aggregate_valid_pairs(self):
        ...
        for from_tf, to_tf in valid_pairs:
            assert TimeframeAggregator.can_aggregate(
                from_tf, to_tf
    ...
    def test_can_aggregate_invalid_pairs(self):
        ...
        invalid_pairs = [
            ("5m", "1m"),
            ("15m", "5m"),
            ("1h", "15m"),  # Reverse direction
            ("1m", "7m"),
            ("5m", "17m"),  # Non-standard timeframes
            ("invalid", "5m"),
            ("1m", "invalid"),  # Invalid timeframes
        ]

        for from_tf, to_tf in invalid_pairs:
            assert not TimeframeAggregator.can_aggregate(
                from_tf, to_tf
...
class TestPerformance:
    ...
    def test_aggregation_performance_large_dataset(self, large_dataset, performance_thresholds):
        ...
        assert (
            execution_time_ms < performance_thresholds["aggregation_time_ms"]
    ...
    def test_memory_usage_aggregation(self, large_dataset):
        ...
        assert (
            memory_increase < 50
        ), f"Memory increased by {memory_increase:.2f}MB during aggregation"
        assert_dataframe_valid(result)
...
class TestParametrizedTests:
    ...
    @pytest.mark.parametrize(
        "from_tf,to_tf",
        [("1m", "5m"), ("1m", "15m"), ("1m", "1h"), ("5m", "15m"), ("5m", "1h"), ("15m", "1h")],
    )
    @pytest.mark.aggregation
