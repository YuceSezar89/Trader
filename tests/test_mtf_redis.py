...
async def test_mtf_cache_flush():
    ...
    cache_count_before = sum(
        1 for tf_stats in stats_before.values() for info in tf_stats.values() if info["exists"]
    )
    ...
    cache_count_after = sum(
        1 for tf_stats in stats_after.values() for info in tf_stats.values() if info["exists"]
    )
