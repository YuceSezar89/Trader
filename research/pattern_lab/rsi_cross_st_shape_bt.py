...
def run():
    ...
    for st in (True, False):
        for kategori in CATEGORIES:
            ...
            s = _pf(sub)
            print(
                f"{str(st):14} {kategori:20} {s.get('n',0):>7} {s.get('wr',0):>6} "
    ...
    s_all = _pf(only_true)
    print(
        f"{'(hepsi)':20} {s_all.get('n',0):>7} {s_all.get('wr',0):>6} "
    ...
    for kategori in CATEGORIES:
        ...
        s = _pf(sub)
        print(
            f"{kategori:20} {s.get('n',0):>7} {s.get('wr',0):>6} "
            f"{s.get('ort_%',0):>8} {s.get('pf',0):>7}"
        )
    ...
    s = _pf(sub)
    print(
        f"{'gövde+alt-fitil':20} {s.get('n',0):>7} {s.get('wr',0):>6} "
        f"{s.get('ort_%',0):>8} {s.get('pf',0):>7}"
    )
    ...
    for name, g in groups.items():
        ...
        if s.get("n", 0) == 0:
            ...
            continue
        print(
            f"{name:32} {s['n']:>7} {s['wr']:>6} {s['avg_usd']:>12} "
