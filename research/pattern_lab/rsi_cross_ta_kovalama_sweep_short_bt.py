"""
Kovalama eşik sweep'inin Short replikasyonu — rsi_cross_ta_kovalama_sweep_bt.py'nin
aynısı, ters yön (24 Tem 2026, kullanıcı isteği).

kovalama(eşik) = (pct_1h<=eşik & slope_1h<0) OR (pct_4h<=eşik & slope_4h<0)
Long'un [60..95] taramasının aynası: [40,35,30,25,20,15,10,5]. Mevcut sabit
eşik Short'ta 20'ydi (Long'un 80'inin aynası).

Kullanım: python -m research.pattern_lab.rsi_cross_ta_kovalama_sweep_short_bt
(önce rsi_cross_ta_triple_combo_short_bt.py çalıştırılmış olmalı — cache'i kullanır)
"""
...
from research.pattern_lab.rsi_cross_ta_triple_combo_short_bt import _CACHE_PATH

_THRESHOLDS = [40, 35, 30, 25, 20, 15, 10, 5]
_MIN_N = 30
_CURRENT_TH = 20
...
def main() -> None:
    ...
    print(f"[all_up=True popülasyonu, Short] n={len(df)}\n")
    ...
    print("=" * 78)
    print("EŞİK TARAMASI (Short, SADECE IS verisiyle)")
    print("=" * 78)
    ...
    for th in _THRESHOLDS:
        ...
        marker = " <- mevcut (20)" if th == _CURRENT_TH else ""
    ...
    print(f"\n=== Karşılaştırma: mevcut sabit eşik (20) tüm veri ===")
    ...
    df["_g"] = best_mask
    _deep_validate(f"Short kovalama eşik={best_th}", best_group, rest, df)
    ...
    summarize(f"Short kovalama eşik={best_th} — fwd_ret% serisi", best_group["fwd_ret"].to_numpy(), best_group["opened_at"], days_span)
