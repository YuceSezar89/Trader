...
from research.pattern_lab.do_open_streak_bt import (  # pylint: disable=wrong-import-position
    DAYS, HORIZON_BARS, MIN_BARS, MTF_CONFIRM_TFS, MTF_STREAK_TARGET,
    _fetch, _do_break_gate, _streak_events,
)
...
from research.pattern_lab.do_break_gauss_sltp_bt import _simulate_exit  # pylint: disable=wrong-import-position
    ...
    for h, l, c, atr_val, i, entry_price in events:
        ...
        for sl in SL_MULTIPLES:
            exit_price, reason = _simulate_exit(
                h, l, c, i, entry_price, atr_val,
                sl_mult=sl, tp_mult=None, breakeven_mult=None,
            )
...
def run():
    ...
    for sym, g in df.groupby("symbol"):
        ...
        target_events = [i for i in events[MTF_STREAK_TARGET]
