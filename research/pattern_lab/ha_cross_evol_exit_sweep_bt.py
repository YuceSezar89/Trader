...
def _simulate_evol_exit_detailed(
...
def run() -> None:
    ...
    b_tum, b_ilk, b_ik = (
        _stats(baseline_rets),
        _stats(baseline_rets[first_mask]),
        _stats(baseline_rets[~first_mask]),
    )
    print(
        f"BASELINE (ATR-tek-TP): PF_tum={b_tum.get('pf',0)} PF_ilk={b_ilk.get('pf',0)} PF_ikinci={b_ik.get('pf',0)}\n"
    )

    print(
        f"{'eşik':>5} {'min_hold':>9} {'PF_tum':>8} {'PF_ilk':>8} {'PF_ikinci':>10} {'evol%':>7} {'floor%':>7}"
    )
    for th in THRESHOLDS:
        for mh in MIN_HOLDS:
            ...
            print(
                f"{th:>5} {mh:>9} {s_tum.get('pf',0):>8} {s_ilk.get('pf',0):>8} "
