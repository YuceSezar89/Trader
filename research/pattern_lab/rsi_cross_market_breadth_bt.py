...
def run():
    ...
    print(f"Piyasa genişliği serisi: {len(breadth_series):,} zaman noktası "
    ...
    for _sym, g in df.groupby("symbol"):
        ...
        for i, direction in filtered_events:
            ...
            clean_events.append((
                direction, h, l, c, atr[i], i, c[i], ts.iloc[i], post_v - pre_v, breadth,
            ))
    ...
    print(f"SABİT (OOS'a uygulanan) genişlik eşikleri: düşük=%{breadth_low:.1f} yüksek=%{breadth_high:.1f}\n")
    ...
    for dir_label, direction in (("LONG+SHORT (birlikte)", None), ("SADECE LONG", "Long"), ("SADECE SHORT", "Short")):
        ...
        _report("VPMV + piyasa genişliği filtresi", [e[:7] for e in vb if e[7] < oos_mid], half_days)
