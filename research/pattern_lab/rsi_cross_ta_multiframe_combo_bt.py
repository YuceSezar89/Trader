"""
TA-uyumu — farklı TF çiftleri + üçlü uyum taraması (23 Tem 2026, kullanıcı isteği).

rsi_cross_allup_ta_combo_fast_bt.py'nin devamı: RSI_Cross(9,24) Long/5m +
all_up=True popülasyonunda, TA'nın (12h kümülatif net%) 4 TF'de (5m/15m/1h/4h)
BAĞIMSIZ hesaplanıp, farklı TF ikilisi/üçlüsü kombinasyonlarının hangisinin
en güçlü ayrım gücü kattığı taranıyor.

Aşama 1: her kombinasyon için hızlı özet (n/WR/PF)
Aşama 2: en iyi 3 kombinasyon için tam derin doğrulama (placebo+split-period)

Kullanım: python -m research.pattern_lab.rsi_cross_ta_multiframe_combo_bt
"""
...
def _placebo(group: pd.DataFrame, rest: pd.DataFrame, full: pd.DataFrame) -> float:
    real_gap = group["fwd_ret"].mean() - rest["fwd_ret"].mean()
    rng = np.random.default_rng(42)
    labels = full["_g"].to_numpy()
    target = full["fwd_ret"].to_numpy()
    count_ge = 0
    for _ in range(_PLACEBO_ITER):
        ...
        if abs(d) >= abs(real_gap):
            count_ge += 1
    return count_ge / _PLACEBO_ITER * 100
...
def main() -> None:
    ...
    print("=" * 78)
    print("AŞAMA 1 — TF ikilisi/üçlüsü taraması (hepsi all_up=True İÇİNDE)")
    print("=" * 78)
    results = []
    ...
    print("\n" + "=" * 78)
    print("AŞAMA 2 — en iyi 3 kombinasyonun tam derin doğrulaması")
    print("=" * 78)
