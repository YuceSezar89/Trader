"""
HA_Cross Long kovalama eşik sweep'i — rsi_cross_ta_kovalama_sweep_bt.py'nin
HA_Cross replikasyonu. Pine script'e 80 (HTML varsayılanı) yazıldı ama HİÇ
kendi verimizle sweep edilmemişti (24 Tem 2026, kullanıcı isteği).

Kullanım: python -m research.pattern_lab.ha_cross_ta_kovalama_sweep_bt
(önce ha_cross_ta_triple_combo_bt.py çalıştırılmış olmalı — cache'i kullanır)
"""
...
from research.pattern_lab.ha_cross_ta_triple_combo_bt import _HA_CACHE_PATH
...
def main() -> None:
    ...
    print(f"[all_up=True popülasyonu, HA_Cross Long] n={len(df)}\n")
    ...
    print("=" * 78)
    print("EŞİK TARAMASI (HA_Cross Long, SADECE IS verisiyle)")
    print("=" * 78)
    ...
    for th in _THRESHOLDS:
        ...
        marker = " <- Pine'a yazılan (80)" if th == _CURRENT_TH else ""
    ...
    print(f"\n=== Karşılaştırma: Pine'a yazılan sabit eşik (80) tüm veri ===")
    ...
    df["_g"] = best_mask
    _deep_validate(f"HA_Cross Long kovalama eşik={best_th}", best_group, rest, df)
    ...
    summarize(f"HA_Cross Long kovalama eşik={best_th} — fwd_ret% serisi", best_group["fwd_ret"].to_numpy(), best_group["opened_at"], days_span)
