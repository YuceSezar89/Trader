...
from research.pattern_lab.rsi_cross_sl_sweep_bt import (  # pylint: disable=wrong-import-position
    SL_MULTIPLES, _rsi_cross_events, _apply_signal_filter,
    _simulate_sl_exit, _signed_ret, _dollar_stats,
)
...
def _report(label: str, events: list, days_span: float) -> None:
    ...
    for direction, h, l, c, atr_val, i, entry_price in events:
        ...
        for sl in SL_MULTIPLES:
            exit_price, _reason = _simulate_sl_exit(direction, h, l, c, i, entry_price, atr_val, sl, HORIZON_BARS)
...
def run():
    ...
    for _sym, g in df.groupby("symbol"):
        ...
        for i, direction in filtered_events:
            ...
            clean_events.append((
                direction, h, l, c, atr[i], i, c[i], ts.iloc[i], kategori, post_v - pre_v,
            ))
    ...
    oos_combined = [e[:7] for e in oos_events
                    if e[8] != "üst-fitil-baskın" and e[9] >= jump_threshold]
