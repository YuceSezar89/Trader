"""
Üçlü kombinasyonun (all_up + TA-kovalama + HA-hizalanması) Short tarafı
replikasyonu — rsi_cross_ta_triple_combo_bt.py'nin (Long) aynısı, ters yön
(24 Tem 2026, kullanıcı isteği).

Short için "güçlü" TA/HA ne demek — Long'un aynadaki görüntüsü:
- TA-base: percentile(net_ta) DÜŞÜK (<=45, yani net_ta olağandışı NEGATİF)
- kovalama: percentile<=20 VE hâlâ DÜŞÜYOR (slope<0) — "hâlâ hızlanan düşüş"
- HA-hizalı: 1h VE 4h Heikin-Ashi mumu İKİSİ DE BEARISH

Kullanım: python -m research.pattern_lab.rsi_cross_ta_triple_combo_short_bt
"""
...
_CACHE_PATH = os.path.join(os.path.dirname(__file__), "_cache_rsi_cross_ta_triple_short.parquet")
_BASE_TH = 45   # Long'un 55'inin aynası: 100-55
_EXTREME_TH = 20  # Long'un 80'inin aynası: 100-80
...
def main() -> None:
    ...
    print("=" * 78)
    print("ÖZET TABLO — sırayla ekleme etkisi (Short)")
    print("=" * 78)
    steps = [
    ...
    print("\n" + "=" * 78)
    print("ÜÇLÜ KOMBİNASYON (Short) — tam derin doğrulama")
    print("=" * 78)
    if len(triple) < 30:
    ...
    df["_g"] = triple_mask
    _deep_validate("Short: TA-base + kovalama + HA (üçlü)", triple, rest, df)
    ...
    summarize("Short üçlü kombinasyon — fwd_ret% serisi", triple["fwd_ret"].to_numpy(), triple["opened_at"], days_span)
