...
pattern = re.compile(r"VOL_AB \| (\w+) \| [\d.]+ \| yonsuz=([\d.]+) yonlu=([\d.]+) fark=(-?[\d.]+)")
...
def rapor(rows, baslik):
    ...
    print(f"{'='*40}")
    print(
        f"Yönsüz  ort={statistics.mean(yonsuz):.1f}  std={statistics.stdev(yonsuz) if len(yonsuz)>1 else 0:.1f}"
    )
    print(
        f"Yönlü   ort={statistics.mean(yonlu):.1f}  std={statistics.stdev(yonlu) if len(yonlu)>1 else 0:.1f}"
    )
    print(
        f"Fark    ort={statistics.mean(farklar):.1f}  min={min(farklar):.1f}  max={max(farklar):.1f}"
    )
