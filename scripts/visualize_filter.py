...
async def build_chart(
    df: pd.DataFrame, symbol: str, interval: str, atr_length: int, factor: float
) -> go.Figure:
    filt = SignalFilter()
    ...
    for _, row in df.iterrows():
        ...
        ref = (
            ref_pair[0]
            if sig == "Long" and ref_pair
            else (ref_pair[1] if sig == "Short" and ref_pair else None)
        )
        bar_time = (
            row["timestamp"].tz_localize(None) if row["timestamp"].tzinfo else row["timestamp"]
        )
        valid = await filt.check(
            sig, float(row["high"]), float(row["low"]), filter_symbol, interval, ind, bar_time
        )
        signals.append(
            {
                "ts": row["timestamp"],
    ...
    for s in signals:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                text=[label],
                textposition="bottom center" if s["sig"] == "Long" else "top center",
                ...
                hovertemplate=(
                    "<b>{} ({})</b><br>H: {:.1f}  L: {:.1f}<br>Ref: {}<extra></extra>".format(
                        s["sig"],
                        "Geçerli" if s["valid"] else "Geçersiz",
                        s["high"],
                        s["low"],
        ...
        if s["ref"] is not None:
            ...
            if len(idx):
                ...
                fig.add_annotation(
                    ...
                    text=f"{s['ref']:.0f}",
                    showarrow=False,
                    font=dict(size=8, color=ref_color),
                    xanchor="left",
                )
