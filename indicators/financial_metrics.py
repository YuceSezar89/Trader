...
def calculate_metrics(df, ref_df=None, beta_window=50, alpha_window=20):
    ...
    if hasattr(df.index, "tz"):
        ...
        if "date" in df.columns:
            ...
            logger.warning(
                "DataFrame'de date sütunu yok ve index datetime değil, metrik hesaplama atlanıyor."
            )
            df["alpha"] = 0.0
            df["beta"] = 0.0
    ...
    if ref_df is not None and not ref_df.empty:
        try:
            ...
            logger.info(
                f"Ortak index uzunluğu: {len(common_index)}, df: {len(df)}, ref_df: {len(ref_df)}"
            )
            if len(common_index) > beta_window:
                ...
                if len(asset_returns) > 1 and len(market_returns) > 1:
                    ...
                    logger.info(
                        f"Rolling beta hesaplandı: min={rolling_beta.min():.4f}, max={rolling_beta.max():.4f}, son={rolling_beta.iloc[-1]:.4f}"
                    )
                    ...
                    rolling_alpha = (
                        asset_returns_alpha - market_returns_alpha * rolling_beta
                    ) * 100
    ...
    df.loc[:, "percent_volatility_diff"] = (
        df["volatility_diff"] / df["volatility"].shift(1).replace(0, 1)
    ) * 100
    ...
    df.loc[:, "omega_ratio"] = np.where(
        ...
        0,
        df["positive_returns"] / (df["negative_returns"] + 1e-6),
    )
    ...
    if ref_df is not None and not ref_df.empty:
        ...
        df["treynor_ratio"] = np.where(
            beta == 0, 0, (df["avg_return"] - risk_free_adjusted) / (beta + 1e-6)
        )
        ...
        if nan_count > len(df) * 0.1:  # %10'dan fazla NaN varsa
            logger.debug(
                f"High NaN count in information_ratio: {nan_count}/{len(df)} ({nan_count/len(df)*100:.1f}%)"
            )
    ...
    df["scaled_avg_normalized"] = (
        ...
        .apply(normalize_series, axis=0, index=df.index)
        .mean(axis=1)
        * 100
    ).fillna(0.0)
    ...
    df.loc[:, "price_stdev_200"] = (
        df["close"].rolling(window=200, min_periods=200).std(ddof=0)
    )  # Pine stdev (population)
    df.loc[:, "zscore_ratio_percent"] = (
        (df["close"] - df["ema200"]) / df["price_stdev_200"].replace(0, np.nan)
    ) * 100
