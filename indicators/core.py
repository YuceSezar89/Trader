...
def calculate_ema(
    df: pd.DataFrame, period: int = Config.EMA_DEFAULT_PERIOD, price_col: str = "close"
) -> pd.Series:
    """
...
def calculate_sma(
    df: pd.DataFrame, period: int = Config.SMA_DEFAULT_PERIOD, price_col: str = "close"
) -> pd.Series:
    """
    ...
    try:
        ...
        if period <= 0:
            raise ValidationError(
                f"Period pozitif olmalı, alınan: {period}", field="period", value=period
            )
        ...
        logger.debug(
            f"RSI hesaplandı: period={period}, price_col={price_col}, valid_values={rsi.notna().sum()}"
        )
        return rsi
...
def calculate_supertrend(df: pd.DataFrame, atr_length: int = 10, factor: float = 3.0) -> tuple:
    """
    ...
    st_line = pd.Series(np.where(direction == -1, lower, upper), index=df.index, dtype=float)
...
def calculate_awesome_oscillator(df, fast_period=5, slow_period=34):
    ...
    ao = (
        median_price.rolling(window=fast_period).mean()
        - median_price.rolling(window=slow_period).mean()
    )
    return ao
...
def calculate_stoch_rsi(
    ...
    stoch_period=Config.RSI_PERIOD_DEFAULT,
    k_period=Config.STOCH_D_PERIOD,
...
def calculate_evol(
    df: pd.DataFrame, rvol_window: int = 20, rank_window: int = 100
) -> "float | None":
    """
