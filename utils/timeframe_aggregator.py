...
class TimeframeAggregator:
    ...
    def can_aggregate(cls, from_tf: str, to_tf: str) -> bool:
        ...
        """
        return (
            from_tf in cls.SUPPORTED_AGGREGATIONS and to_tf in cls.SUPPORTED_AGGREGATIONS[from_tf]
        )
    ...
    @classmethod
    def get_closing_timeframes(
        cls, next_open_time_ms: int, tf_list: Optional[List[str]] = None
    ) -> List[str]:
    ...
    def aggregate_ohlcv(cls, df: pd.DataFrame, from_tf: str, to_tf: str) -> pd.DataFrame:
        ...
        try:
            ...
            if first_aligned_idx is None:
                logger.warning(
                    f"Boundary alignment bulunamadı: {to_tf} için uygun başlangıç yok. Boş döndürülüyor."
                )
                return pd.DataFrame()
            ...
            if first_aligned_idx > 0:
                ...
                logger.info(
                    f"Boundary alignment uygulandı: ilk hizalı indeks={first_aligned_idx} | ilk ts={first_ts}"
                )
        except Exception as e:
            logger.warning(
                f"Boundary alignment sırasında hata: {e}. Devam ediliyor (mevcut sırayla)."
            )
        ...
        logger.info(
            f"Aggregation tamamlandı: {len(df_sorted)} -> {len(result_df)} bars ({from_tf} -> {to_tf})"
        )
    ...
    def aggregate_to_multiple_timeframes(
        cls, df: pd.DataFrame, from_tf: str, target_timeframes: List[str]
    ...
    def create_test_data(
        cls, bars: int = 50, start_time: Optional[datetime] = None
    ) -> pd.DataFrame:
        """
