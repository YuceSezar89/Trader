...
class SimpleBacktest:
    ...
    def run_backtest(
        ...
        try:
            ...
            conn.commit()
            logger.info(
                f"Backtest tamamlandı: {len(trades)} işlem, Win Rate: {results['win_rate']:.2f}%"
            )
    ...
    def _get_price_at_time(
        self, cursor, symbol: str, target_time, interval: str
    ) -> Optional[float]:
        """
    ...
    def _analyze_signal(self, cursor, signal: Dict) -> Optional[Dict]:
        ...
        if pnl_pct <= -1.5:  # Stop loss
            ...
            pnl_pct = -1.5
            exit_price = (
                entry_price * (1 - 0.015) if signal_type == "AL" else entry_price * (1 + 0.015)
            )
        elif pnl_pct >= 2.0:  # Take profit
            exit_reason = "TAKE_PROFIT"
            pnl_pct = 2.0
            exit_price = (
                entry_price * (1 + 0.02) if signal_type == "AL" else entry_price * (1 - 0.02)
            )
    ...
    def _calculate_results(
        self, trades: List[Dict], days_back: int, intervals: Optional[List[str]]
    ) -> Dict:
