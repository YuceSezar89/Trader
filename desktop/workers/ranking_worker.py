    ...
    def _compute(self) -> list:
        ...
        for sym in symbols:
            ...
            if tf_zscores:
                ...
                z_confluence = round(
                    sum(tf_zscores[tf] * _TF_WEIGHTS[tf] for tf in tf_zscores) / total_zw, 2
                )
            else:
                z_confluence = None
            ...
            r_score = (
                self._r_score(df_1h) if df_1h is not None and len(df_1h) >= _R_PERIOD else None
            )
        ...
        for data in scores.values():
            data["vs_btc"] = (
                round(data["combined"] - btc_combined, 1) if btc_combined is not None else None
            )
