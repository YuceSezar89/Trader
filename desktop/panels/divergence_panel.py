...
from desktop.models.divergence_model import (
    COL_RANK,
    COL_SYMBOL,
    COL_TIME,
    COL_VPMV,
    DivergenceModel,
    DivergenceProxyModel,
)
...
def _make_view(positive: bool) -> tuple[QTableView, DivergenceModel, DivergenceProxyModel]:
    model = DivergenceModel(positive)
    proxy = DivergenceProxyModel()
    ...
    hh.setSectionResizeMode(COL_VPMV, QHeaderView.ResizeMode.Interactive)
            ...
            items.append(
                {
                    ...
                    "time_str": time_str,
                    "rank_delta": rank_deltas.get(symbol, 0),
                    "is_stale": is_stale,
                }
            )
